...
from src.qmann.hybrid.training_protocols import AdvancedTrainingProtocols, PerformanceBenchmark
...
class TestQMANNIntegration:
    ...
    def test_classical_quantum_interface(self, config):
        ...
        for i in range(classical_output.shape[1]):  # For each time step
            ...
            if len(classical_vector) > 8:
                ...
                classical_vector = np.pad(classical_vector, (0, 8 - len(classical_vector)))
    ...
    def test_model_persistence(self, config, tmp_path):
        ...
        output2, _, _ = model2(input_data, use_quantum_memory=False)  # Disable quantum for deterministic test
    ...
    def test_performance_benchmarking(self, config, sample_data):
        ...
        quantum_output, _, quantum_info = quantum_model(input_data, use_quantum_memory=True)
        quantum_time = time.time() - start_time
        
        # Classical model timing
        start_time = time.time()
        classical_output, _, classical_info = classical_model(input_data, use_memory=True)
