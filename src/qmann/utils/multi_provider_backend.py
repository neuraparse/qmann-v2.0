...
try:
    ...
    warnings.warn(
        "AWS Braket provider not available. Install with: pip install qiskit-braket-provider"
    )
...
try:
    ...
    warnings.warn(
        "Rigetti provider not available. Install with: pip install qiskit-rigetti"
    )
...
class MultiProviderBackendManager:
                ...
                try:
                    ...
                    backends.append(
                        BackendInfo(
                            ...
                            and not status.status_msg == "offline",
                            ...
                            backend_version=getattr(
                                config, "backend_version", "unknown"
                            ),
                        )
                    )
                except Exception as e:
            ...
            for backend in ionq_backends:
                backends.append(
                    BackendInfo(
                        ...
                        backend_version=(
                            "aria-1" if "aria" in backend.name().lower() else "harmony"
                        ),
                    )
                )
        except Exception as e:
            ...
            for backend in braket_backends:
                backends.append(
                    BackendInfo(
                        ...
                        queue_length=0,
                        cost_per_shot=(
                            0.00030 if "ionq" in backend.name.lower() else 0.00035
                        ),
                        gate_fidelity=0.999,
    ...
    def get_best_backend(
        ...
        # Score and sort
        candidates.sort(
            key=lambda b: b.score(prefer_hardware, cost_weight=0.3), reverse=True
        )
        ...
        self.logger.info(
            f"Selected backend: {best_info.name} ({best_info.provider.value})"
        )
        ...
        if backend_info.provider == QuantumProvider.IBM_QUANTUM:
            with Session(service=self._ibm_service, backend=backend) as session:
                ...
                self.stats["total_cost"] += (
                    backend_info.cost_per_shot * shots * len(circuits)
                )
    ...
    def execute_batch(
        ...
        if backend_info.provider == QuantumProvider.IBM_QUANTUM:
            with Batch(service=self._ibm_service, backend=backend) as batch:
                ...
                self.stats["total_cost"] += (
                    backend_info.cost_per_shot * shots * total_circuits
                )
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            **self.stats,
            "available_providers": [
                provider.value
                for provider, available in self.initialize_providers().items()
                if available
            ],
...
def get_quantum_backend(
    num_qubits: int = 1, prefer_hardware: bool = False, provider: Optional[str] = None
) -> Tuple[Backend, BackendInfo]:
    """
