...
class BenchmarkResult:
    ...
    def summary(self) -> str:
        ...
        if self.task_type == "classification":
            lines.extend([
                f"Accuracy: {self.accuracy:.4f}" if self.accuracy else "Accuracy: N/A",
                f"Precision: {self.precision:.4f}" if self.precision else "Precision: N/A",
                f"Recall: {self.recall:.4f}" if self.recall else "Recall: N/A",
                f"F1-Score: {self.f1_score:.4f}" if self.f1_score else "F1-Score: N/A",
            ])
        ...
        if self.quantum_fidelity is not None:
            lines.extend([
                ...
                f"Circuit Depth: {self.circuit_depth}" if self.circuit_depth else "Circuit Depth: N/A",
                f"Gate Count: {self.gate_count}" if self.gate_count else "Gate Count: N/A",
            ])
...
class PerformanceBenchmark:
    ...
    def _benchmark_model(
        ...
        # Setup training
        criterion = nn.MSELoss() if task_info["type"] == "regression" else nn.CrossEntropyLoss()
        ...
        for epoch in range(num_epochs):
            ...
            for batch_data, batch_targets in train_loader:
                ...
                if hasattr(model, 'forward') and 'use_quantum_memory' in model.forward.__code__.co_varnames:
                    outputs, _, quantum_info = model(batch_data, use_quantum_memory=True)
                    if quantum_info and result.quantum_fidelity is None:
                        result.quantum_fidelity = quantum_info.get("quantum_fidelity", 0.0)
                else:
        ...
        if task_info["type"] == "classification":
            ...
            result.precision = precision_score(true_classes, pred_classes, average='weighted', zero_division=0)
            result.recall = recall_score(true_classes, pred_classes, average='weighted', zero_division=0)
            result.f1_score = f1_score(true_classes, pred_classes, average='weighted', zero_division=0)
        ...
        if hasattr(model, 'quantum_memory'):
            result.quantum_memory_usage = getattr(model.quantum_memory, 'memory_usage', 0.0)
    ...
    def _generate_synthetic_classification(self) -> Tuple[DataLoader, DataLoader, Dict[str, Any]]:
    ...
    def _generate_synthetic_regression(self) -> Tuple[DataLoader, DataLoader, Dict[str, Any]]:
    ...
    def _generate_continual_learning_tasks(self) -> Tuple[DataLoader, DataLoader, Dict[str, Any]]:
        ...
        for task_id in range(n_tasks):
            ...
            X_task = torch.randn(n_samples_per_task, sequence_length, n_features) + task_id
    ...
    def _generate_memory_intensive_task(self) -> Tuple[DataLoader, DataLoader, Dict[str, Any]]:
    ...
    def _create_transformer_baseline(self, task_info: Dict[str, Any]) -> nn.Module:
        ...
        class TransformerBaseline(nn.Module):
            ...
            def forward(self, x):
                ...
                output = self.fc(self.dropout(transformer_out[:, -1, :]))  # Use last timestep
                return output
    ...
    def generate_comparison_report(self) -> str:
        ...
        report_lines = [
            "QMANN Benchmark Comparison Report",
            "=" * 50,
            ""
        ]
        ...
        for dataset_name, dataset_results in datasets.items():
            ...
            baseline_results = [r for r in dataset_results if "Baseline" in r.model_name]
            ...
            if qmann_results and baseline_results:
                ...
                if qmann_result.task_type == "classification":
                    ...
                    baseline_metrics = [r.accuracy for r in baseline_results if r.accuracy is not None]
                elif qmann_result.task_type == "regression":
                    metric_name = "R² Score"
                    qmann_metric = qmann_result.r2_score
                    baseline_metrics = [r.r2_score for r in baseline_results if r.r2_score is not None]
                else:
                    continue
    ...
    def get_summary_statistics(self) -> Dict[str, Any]:
        ...
        if qmann_results:
            ...
            quantum_fidelities = [r.quantum_fidelity for r in qmann_results if r.quantum_fidelity is not None]
