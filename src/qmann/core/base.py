...
class QMANNBase(abc.ABC):
    ...
    def __init__(self, config: QMANNConfig, name: str = None):
        ...
        self.logger.info(f"Initialized {self.name} with config: {type(config).__name__}")
...
class QuantumComponent(QMANNBase):
    ...
    def execute_circuit(self, circuit, shots: int = None):
        ...
        self.metrics['circuit_depth'] = max(self.metrics['circuit_depth'], circuit.depth())
...
class ClassicalComponent(QMANNBase, nn.Module):
    ...
    def initialize(self) -> None:
        ...
        self.logger.info(f"{self.name} initialized with {self.metrics['parameters']} parameters")
...
class HybridComponent(QMANNBase):
    ...
    def sync_components(self) -> None:
        ...
        self.logger.debug(f"Synchronized components (operation #{self.metrics['sync_operations']})")
