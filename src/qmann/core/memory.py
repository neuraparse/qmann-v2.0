...
class ClassicalMemoryInterface(MemoryInterface):
    ...
    def read(self, query: torch.Tensor, k: int = 1) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        similarities = torch.cosine_similarity(
            query.unsqueeze(0), 
            self.memory_matrix[:self.current_size], 
            dim=1
        )
    ...
    def forward(self, query: torch.Tensor, k: int = 1) -> Tuple[torch.Tensor, torch.Tensor]:
