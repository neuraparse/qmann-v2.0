...
try:
    ...
    raise ImportError("Qiskit is required for QMANN. Install with: pip install qiskit>=2.1.0")
...
try:
    ...
    if torch.cuda.is_available():
        ...
        print("QMANN: Running in CPU mode. For GPU acceleration, ensure CUDA is properly installed.")
except ImportError:
...
try:
    ...
    try:
        ...
        print(f"QMANN: IBM Quantum access confirmed - {len(backends)} backends available")
    except Exception:
        print("QMANN: IBM Quantum access not configured. Use QiskitRuntimeService.save_account() to set up.")
except ImportError:
    print("QMANN: IBM Quantum Runtime not available. Install with: pip install qiskit-ibm-runtime")
